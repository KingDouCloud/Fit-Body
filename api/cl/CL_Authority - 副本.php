...
class Authority
{
	...
	private function db_update($userid,$md5psw)
	{
		...
		$str = "update fitbody.tb_login set cl_psw='".$md5psw."' where cl_userid='".$userid."'";
	...
	{
		require("../../db/connect.php");
		$str = "insert into fitbody.tb_login value('".$userid."','".$psw."','".$authority."')";
		$sqlclass = mysql_query($str,$conn);
		require("../../db/connect_close.php");
	}
	...
	{		
		//查詢這個用戶的記錄		
		require("../../db/connect.php");
		$str = "select fitbody.tb_login.cl_userid, fitbody.tb_login.cl_psw, fitbody.tb_login.cl_authority ";
		$str .= "from fitbody.tb_login,fitbody.tb_userinfo ";
		$str .= "where fitbody.tb_login.cl_userid=";
		$str .= "(select cl_userid from fitbody.tb_userinfo where cl_username = '".$username."')";
		$sqlclass = mysql_query($str,$conn);
		if($sqlclass)
	...
	{		
		...
		require("../../db/connect.php");
		$str = "select fitbody.tb_login.cl_userid,fitbody.tb_login.cl_psw,fitbody.tb_login.cl_authority ";
		$str .= "from fitbody.tb_login ";
		$str .= "where fitbody.tb_login.cl_userid='".$userid."'";
		$sqlclass = mysql_query($str,$conn);
		if($sqlclass)
			$result = mysql_fetch_array($sqlclass);
		require("../../db/connect_close.php");	
	...
	{		
		//查詢這個用戶的記錄		
		require("../../db/connect.php");
		$str = "select fitbody.tb_login.cl_userid,fitbody.tb_login.cl_psw ";
		$str .= "from fitbody.tb_login ";
		$str .= "where fitbody.tb_login.cl_userid='".$userid."'";
		$sqlclass = mysql_query($str,$conn);
		if($sqlclass)
